...
export function CStep3OTP({ formData, onNext }: StepProps<CompanyOnboardingData>) {
  const [digits, setDigits] = useState(['', '', '', '', '', '']);
  const isValid = digits.join('') === OTP_DEMO_CODE;

  return (
    <motion.div key="c3" custom={1} variants={variants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }} className="space-y-8">
